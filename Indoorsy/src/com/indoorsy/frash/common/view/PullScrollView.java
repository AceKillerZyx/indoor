...
public class PullScrollView extends ScrollView {
	...
	private PulllScrollHeaderView headerView;
	private PulllScrollFooterView footerView;
	...
	private void addheaderView() {
		headerView = new PulllScrollHeaderView(mContext);
	...
	private void addfooterView() {
		footerView = new PulllScrollFooterView(mContext);
		measureView(footerView);
		footContentHeight = footerView.getMeasuredHeight();
		footerView.setPaddingButtom(-1 * headContentHeight);
...
//			Log.i(TAG, "在down时候记录当前位置startY[onInterceptTouchEvent]" + startY);
	...
	private void changeheaderViewHeight(int tempY) {
		...
		if (pullState == PULL_DOWN_STATE || pullState == RELEASE_TO_REFRESH) {
			pullState = headerView.setPadding(headContentHeight - 90 /* 这里很重要 -90 是为了减去标题栏的高度，否则下拉刷新的时候会导致下拉过长*/
					, -1* headContentHeight + tempY / RATIO);
		}
	}
