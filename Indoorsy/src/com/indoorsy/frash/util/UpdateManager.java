...
public class UpdateManager implements HttpListener{
	...
	private void showDownloadDialog() {
		...
		View v = inflater.inflate(R.layout.update_system_progress, null);
	...
	private int getVersionCode(Context mcontext) {
		...
		try {
			...
			//ToastUtil.toast(mcontext, "当前版本号为:"+versionCode);
	...
	private void jiazai() {
		...
		HttpParam httpParam = new HttpParam(ReleaseConfigure.COMMON_APK_UPDATE, false); // get
		...
		if(result != null && result.getData() != null && StringUtil.isGoodJson(result.getData())){
			...
			if (commonResult.validate()) {
				...
				if(jsonArray != null && jsonArray.size() > 0){
					...
					double versionnumber = jsonObject.getDoubleValue(Constants.VERSIONNUMBER);
