...
public class ProductListActivity2 extends BasicActivity implements OnItemClickListener{

	private static final String TAG = ProductListActivity2.class.getSimpleName();
	...
	private void initExpandTabListener() {
		...
		viewMiddle.setOnSelectListener(new ViewMiddle.OnSelectListener() {
			...
			public void getValue(String showText) {
				...
				//ToastUtil.toast(getApplicationContext(), showText);
			}
		});
...
//		Toast.makeText(ProductListActivity.this, showText, Toast.LENGTH_SHORT).show();
		//getTypeid = categoryList.get(position).getCatId();
		switch (onClickType) {
			case 0:
				if("分类筛选".equals(showText)){
					//getTypeid = 0;
					HomePageExpandTabView.setTitle(" ",1);
				...
				//	for(int i = 0;i < categoryList.size();i++){
					...
					HomePageExpandTabView.setTitle(viewRight.getShowText(),1);
			...
			//	↓
				//for(int i = 0;i < categoryList.size();i++){
					//if(categoryList.get(i).getCatName().equals(showText)){
						//ToastUtil.toast(getApplicationContext(),""+ categoryList.get(position).getCatId());
						//getTypeid = categoryList.get(position).getCatId();
					//}
				//}
				//getTypeid=82;
...
//						ToastUtil.toast(getApplicationContext(),commonResult.getErrMsg());
	...
	private void initType(JSONArray jsonArray){
		...
		catStrings[0] = "分类筛选";
		if (catStrings[0].equals("分类筛选")) {
			HomePageExpandTabView.setTitle(" ", 1);
			viewRight.setVisibility(View.GONE);
		//	HomePageExpandTabView.setv
			//ToastUtil.toast(getApplicationContext(), "谁谁谁水水水水");
		}else {
			HomePageExpandTabView.setTitle("综合排序", 1);
			viewRight.setVisibility(View.VISIBLE);
		}
